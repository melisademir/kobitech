    ...
    <KobiLayout>
      <div className="space-y-6">
        ...
        <div className="flex gap-2 overflow-x-auto pb-2">
          {tabs.map((t, i) => (
            <button key={t} onClick={() => setActiveTab(i)} className={`whitespace-nowrap px-4 py-2 rounded-full text-sm font-medium transition-all ${activeTab === i ? "gradient-primary text-primary-foreground" : "bg-card border border-border text-muted-foreground hover:border-primary/30"}`}>
              {t}
            </button>
          ))}
        </div>
